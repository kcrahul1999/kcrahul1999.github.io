...
Next I standardized the multiple regression coefficients. The first step was to add a column so that the model can fit the 'satisfaction' variable when all predictors are zero. Then I made the OLS regression model, making sure to exclude the intercept the at end. 
```{python}
...
#Take out intercept and extract the rest
