...
From the head and describe function we see that the data has transactions from consumers at a yogurt place.
...
I start by adding binary variables to indicate the first 3 products; and then I "pivoted" and "melted" from a wide to long dataframe. 

```{python}
#melt to long format for 'f', 'p', and 'y'
id_vars = ['id']
...
print("New Market Shares after $0.10 price increase for Yogurt 1:", new_market_shares)
...
To estimate a MNL model omitting the following levels to avoide multicollinearity I've chosen to use the statsmodels. Unfortunatly I could not get this to work. Therefore I decided to use the scikit-learn instead.
...
We can see that the model performs well in predicted true negatives but not as well in predicting true positives.
...
The feature that is most prefered is Price as its the most significant factor because of the large negative coefficient. 
